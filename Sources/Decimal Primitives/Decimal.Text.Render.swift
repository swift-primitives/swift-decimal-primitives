...
extension Decimal.Text.Render: Sendable where Value: Sendable { }

extension Decimal.Text.Accessor where Value == Decimal.Format64 {
    public var render: Decimal.Text.Render<Value> {
        Decimal.Text.Render(base)
    }
}

extension Decimal.Text.Render where Value == Decimal.Format64 {
    ...
    public func append(
        ...
        let count = temp.withUnsafeMutableBufferPointer { ptr in
            into(ptr, style: style)
        }
        buffer.append(contentsOf: temp[0..<count])
    }
}

extension Decimal.Text.Accessor where Value == Decimal.Format32 {
    public var render: Decimal.Text.Render<Value> {
        Decimal.Text.Render(base)
    }
}

extension Decimal.Text.Render where Value == Decimal.Format32 {
    /// Render into preallocated buffer, returns bytes written
    public func into(
        _ buffer: UnsafeMutableBufferPointer<UInt8>,
        style: Decimal.Text.Style = .plain
    ) -> Int {
        // TODO: Implement decimal to string rendering
...
extension Decimal.Text.Accessor where Value == Decimal.Format128 {
    public var render: Decimal.Text.Render<Value> {
        Decimal.Text.Render(base)
    }
}

extension Decimal.Text.Render where Value == Decimal.Format128 {
    /// Render into preallocated buffer, returns bytes written
    public func into(
        _ buffer: UnsafeMutableBufferPointer<UInt8>,
        style: Decimal.Text.Style = .plain
